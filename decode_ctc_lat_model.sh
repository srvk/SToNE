...
if [ $# != 4 ]; then
   ...
   echo "Usage: steps/decode_ctc.sh [options] <graph-dir> <data-dir> <decode-dir> <model_dir>"
...
[ -z "$subsample_feats" ] && subsample_feats=`cat $model/subsample_feats 2>/dev/null` || subsample_feats=false
[ -z "$splice_feats" ] && splice_feats=`cat $model/splice_feats 2>/dev/null` || splice_feats=false
